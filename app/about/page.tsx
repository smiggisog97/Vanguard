...
export default function AboutPage() {
  return (
    <div className="mx-auto max-w-[1200px] px-6 py-[61px]">
      ...
      <section className="reveal-section mt-10 grid grid-cols-1 gap-10 lg:grid-cols-[1fr_320px]">
        <div>
          ...
          <h2 className="mt-10 font-display text-[24px] text-ink">Mission</h2>
          <p className="mt-4 text-[16px] leading-[1.65] text-driftwood">
            {founderBio.mission}
          </p>
        </div>
      ...
      <section className="reveal-section mt-12">
        ...
        <ul className="mt-4 space-y-2">
          {founderBio.education.map((e) => (
            <li key={e} className="text-[15px] text-ink">
              {e}
            </li>
          ))}
        </ul>
      ...
      <section className="reveal-section mt-12">
        ...
        <ul className="mt-4 space-y-2">
          {founderBio.publications.outlets.map((o) => (
            <li key={o} className="text-[15px] text-ink">
              {o}
            </li>
          ))}
        </ul>
          ...
          <div>
            ...
            <ul className="mt-2 space-y-1">
              {founderBio.speaking.map((s) => (
                <li key={s} className="text-[15px] text-ink">
                  {s}
                </li>
              ))}
            </ul>
          ...
          <div>
            ...
            <ul className="mt-2 space-y-1">
              {founderBio.media.map((m) => (
                <li key={m} className="text-[15px] text-ink">
                  {m}
                </li>
              ))}
            </ul>
      ...
      <section className="reveal-section mt-12">
        ...
        <ul className="mt-4 space-y-2">
          {founderBio.institutionalRoles.map((r) => (
            <li key={r} className="text-[15px] text-ink">
              {r}
            </li>
          ))}
        </ul>
